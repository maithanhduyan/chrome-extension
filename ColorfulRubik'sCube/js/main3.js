...
var savedSequence = '';//chris added
var lastMove = '';//chris added
var SAVEDSEQ_INIT;//chris added
var initializedSequence = false;//chris added
var wasClickedToSolve = false;//chris added
var rotatingCube = false;//chris added
var movesCount = 0;//chris added
var isSolved = false;//chris added
...
cube.addEventListener('click', function(evt) {
	//Chris ... is this the click to rotate component?
...
cube.audioList = [];
cube.audio = 0;
ERNO.RED.hex = '#f44d9c';//'#DC422F'
ERNO.WHITE.hex = '#FFF';
ERNO.BLUE.hex = '#00c1cf';//'#3D81F6'
ERNO.GREEN.hex = '#98ff98';//'#009D54'
ERNO.ORANGE.hex = '#ffa902';//'#FF6C00'
ERNO.YELLOW.hex = '#FFE135';//'#FDCC09'
ERNO.COLORLESS.hex = '#cdcdcd';//'#444'
...
window.setTimeout(init, 100);//Chris modified
function getSavedSequenceSplit(){
	//The code was written in a way the requires LOGO_SEQUENCE and WCA_SCRAMBLE_SHORT to both be non-empty when initializing.
	//For the above reason, and because we don't want to show really long queues to the user upon opening the extension, this
	//will return the value [LOGO_SEQUENCE, WCA_SCRAMBLE_SHORT]
	//WCA_SCRAMBLE_SHORT will be 10 moves at most since those are visualized upon opening the menu; all others go to LOGO_SEQUENCE 
	//if there are ten or fewer moves then one move will be hidden so that LOGO_SEQUENCE isn't empty
	var split = ['',''];
	if (savedSequence == null || savedSequence == ''){
		SAVEDSEQ_INIT = split;
	}else if(savedSequence.length > 10){
		split[0] = savedSequence.substring(0, savedSequence.length - 11);
		split[1] = savedSequence.substring(savedSequence.length - 11);
	}else{
		split[0] = savedSequence.substring(0, 1);
		split[1] = savedSequence.substring(1);
	}
	SAVEDSEQ_INIT = split;
}
//Chris added...
function init(){
	...
	//Let us see if there is a stored value from before; if so, mute audio and make all but the last ten moves; if there are less than ten
	//moves, use the oldest move here and the remaining moves in the visual move; this is necessary for the cube to initialize correctly
	chrome.storage.sync.get(['rubiksSolved'], function(result) {
		$(".solvecounter").text(result.rubiksSolved);
	});
	chrome.storage.sync.get(['movesCount'], function(result) {
		
		movesCount = result.movesCount == null || result.movesCount == '' ? 0 : result.movesCount;
		chrome.storage.sync.set({'movesCount':movesCount});
		$(".movescounter").text(movesCount);
	});
	chrome.storage.sync.get(['sequenceToRestore'], function(result) {
          console.log('Value currently is ' + result.sequenceToRestore);//chris ... remove
		  console.log('Value as string...' + JSON.stringify(result));
		  
		 if (result.sequenceToRestore == null || result.sequenceToRestore == ''){
			 //If null, undefined, or empty then set up the default value
			 savedSequence = 'lludLrYzbeDxxxYmxR';//todo create a random sequence of 10
			 
			 chrome.storage.sync.set({'sequenceToRestore':savedSequence});
		 }else{
			 savedSequence = result.sequenceToRestore;
		 }
		 
		 getSavedSequenceSplit();
		 setupLogo();		 
	});	
}
function setupLogo() {
  ...
  var LOGO_SEQUENCE = SAVEDSEQ_INIT[0];//Chris...original order... this is the initial shuffle sequence, before the added visual sequence...default is zzxLFFRDuFLUrl
									//This cannot be an empty string at first, else the cube doesn't show
...
function initiateAudio() {
  ...
  cube.addEventListener('onTwistComplete', function(e) {
	  console.log("onTwistComplete function(e)");
    cube.audio.play(cube.audioList[
        Math.floor(Math.random() * (cube.audioList.length - 1))]);
  });
}
function startInteractiveCube() {
	console.log("startInteractiveCube");
	initializedSequence = true;	//Chris added to indicate when twist and immediateTwist should start recording new moves-and ignore initial moves that reset the cube to last state
  ...
  cube.moveCounter = 0;
  cube.addEventListener('onTwistComplete', function(e) {
	  //console.log("onTwistComplete...");
	  //Chris ... this is a good place for code that relies on a twist of the cube/row/column and not a slight tween back into the previous position 
	  console.log("OnTwistComplete");
    if (cube.undoing) {
      cube.moveCounter++;
	  //console.log("cube.mvc++");//chris added...we don't have an undo but this needs to be fixed if we ever do
    }
    if (moveCounter) {
      moveCounter.innerText = moveCounter.textContent = movesCount;//chris modified to use our custom count cube.moveCounter;
    }
    postParentMessage({'moves': cube.moveCounter});
    if (cube.isSolved()) {//Chris ... this checks for a solved cube
		console.log("The cube is solved! Need to reset the database for next time");
		savedSequence = '';
		chrome.storage.sync.set({'sequenceToRestore':savedSequence});
		chrome.storage.sync.set({'movesCount':0});
		if(!wasClickedToSolve){
			//If the user genuinely solved the puzzle, ie did not rely on the solve button, then update the saved value and update the counter
			chrome.storage.sync.get(['rubiksSolved'], function(result) {
				var solveCnt = result.rubiksSolved != null ? result.rubiksSolved + 1 : 1;			 
				console.log("SolveCount:"+solveCnt+",rubSo(before):"+result.rubiksSolved);
				chrome.storage.sync.set({'rubiksSolved':solveCnt});
				//Update the UI	
				$(".solvecounter").text(solveCnt);
				console.log("SolveCount:"+solveCnt+",rubSo:"+result.rubiksSolved);
			});
		}
		playAudio('solve');
		isSolved = true;
...
function scrambleCube() {
  ...
  .to({
        ...
        y: 0,
        z: 0
      }, 1500)//Chris time it take to animate cube from bg to fg at the beginning 
  ...
  .to({
        ...
        z: 0
      }, 3000)//Chris time it take to animate cube from bg to fg at the beginning 
  .easing(TWEEN.Easing.Quartic.InOut)
  .start(cube.time);
  cube.twistDuration = 200;//chris modified from 120 ... initial shuffle speed per each twist
  
  //Chris added for storing the game so that it is always in the same state upon re-opening
  
  
  
  var WCA_SCRAMBLE_SHORT = SAVEDSEQ_INIT[1];//Chris...original order... maybe modify this to stop initial twist if saved...default: ddurrdllrBffDUbffdurfdUbll
							  //This cannot be empty at first else the cube doesn't unlock
  cube.twistCountDown =
      WCA_SCRAMBLE_SHORT.length + cube.twistQueue.history.length;
	  //console.log('ScrambleShort:'+WCA_SCRAMBLE_SHORT);//chris
	  //console.log('Cube.q.hist:'+ JSON.stringify(cube.twistQueue.history));//chris
  ...
  ////Chris added
  //solveCube(2000);
  //cube.addEventListener('onTwistComplete', solveCube(5000));
